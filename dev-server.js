...
class DevServer {
  ...
  setupRoutes() {
    const serveManifest = (req, res) => {
      ...
      console.log(`Serving manifest to ${req.ip} from: ${manifestPath}`);
    ...
    // Serve manifest.json on root / and /manifest.json
    this.app.get("/", serveManifest);
    this.app.get("/manifest.json", serveManifest);

    // Serve individual provider files (both with and without /dist prefix)
    const serveProviderFile = (req, res) => {
    ...
    this.app.get("/dist/:provider/:file", serveProviderFile);
    this.app.get("/:provider/:file", (req, res, next) => {
      // If it's a known endpoint like /status, /providers, skip to next
      if (["status", "providers", "health", "manifest.json"].includes(req.params.provider)) {
        return next();
      }
      serveProviderFile(req, res);
    });
